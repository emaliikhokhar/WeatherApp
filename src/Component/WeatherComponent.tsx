...
const WeatherComponent = ({searched, API_Key}: IProps) => {
    const [cityData, setCityData] = useState<any>();
    const [load, setLoad] = useState<boolean>(true);
    useEffect(() => {
        let response;
        async function FetchMyAPI() {
            response = await fetch(`https://api.weatherapi.com/v1/current.json?key=${API_Key}&q=${searched}&aqi=no`)
    ...
    console.log(searched, "in weather componenet")
    return (
        <div>
            {
